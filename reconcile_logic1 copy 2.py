...
def extract_current_account_entries_from_s3(key: str) -> List[Dict]:
    ...
    entries = []
    current_entry = None  # No active entry yet
    ...
    for line in lines:
        if re.match(r"\d{4}-\d{2}-\d{2}", line):
            ...
            # Only process lines if we've started an entry with a date
            if re.match(r"^INV|^RET|^Invoice|^Amazon|^Brightspace|^Adobe|^FastNet|^Payroll|^City|^Invoice INV", line):
                current_entry["reference"] += " " + line.strip()
 
            elif re.match(r"^\d{1,3}(,\d{3})*\.\d{2}$", line):
                if not current_entry["credit"]:
                    current_entry["credit"] = line
                elif not current_entry["balance"]:
                    current_entry["balance"] = line
 
            else:
                current_entry["description"] += f" {line}" if current_entry["description"] else line
        else:
            print(f"⚠️ Ignoring line before any date: {line}")
 
    if current_entry:
        entries.append(current_entry)
 
    # Format into final structured entries
    parsed_entries = []
    for e in entries:
        try:
            amount = float(e["credit"].replace(",", "")) if e["credit"] else 0.0
            balance = float(e["balance"].replace(",", "")) if e["balance"] else 0.0
            entry_type = "credit" if amount > 0 else "debit"
            parsed_entries.append({
                "date": e.get("date", ""),
                "description": e.get("description", "").strip(),
                "reference": e.get("reference", "").strip(),
                "amount": amount,
                "balance": balance,
                "type": entry_type
            })
        except Exception as err:
            print(f"⚠️ Failed to parse entry: {e} due to {err}")
 
    if not parsed_entries:
        return [{
            "parsing_failed": True,
            "file": key.split("/")[-1],
            "reason": "Could not extract valid structured data from current account statement"
        }]
...
""")
    ...
    def log_section(title, entries, is_matched=False):
        if entries:
            ...
            summary_lines.append("")
    log_section("🏦 All Current Account Entries:", valid_statements)
    log_section("🧾 All Invoices:", valid_invoices)
